...
## 3. UNIVERSAL PRODUCT ENGINE

### The Problem
Current hardcoded categories (Protein, BCAA) prevent other businesses from using the software.

### The Solution: Dynamic Config
1.  **Category Repo**: Allows users to create their own category tree.
2.  **Custom Fields**: Uses a JSON column in the `products` table to store industry-specific data (e.g., "Flavor" for Supplements, "Thread Size" for Hardware).
3.  **Business Type Presets**: A setup wizard that seeds the database with categories based on the user's business type (Marine, Hardware, Retail, etc.).
...
### Phase 3: Cloud & Multi-User (Weeks 9-14)
-   NestJS Backend implementation.
-   Sync Engine integration.
-   Multi-user login and roles.

### Phase 4: AI Intelligence & Mobile (Weeks 15-22)
-   **Predictive Analytics**: AI-driven stock forecasting.
-   **Mobile App**: Field staff order booking and reports.
-   **E-commerce Sync**: Shopify/WooCommerce integration.

### Phase 5: Global Scalability (Month 6+)
-   **Multi-Store/Franchise**: Centralized chain management.
-   **Global Compliance**: Multi-currency & International Tax.
-   **Open API**: 3rd party ecosystem.
